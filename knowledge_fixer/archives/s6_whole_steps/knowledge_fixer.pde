...
void addEdge(String fromLabel, String toLabel) {
  ...
  Node to1 = findNode(toLabel);
  Edge e =new Edge(from, to1);
...
class Edge {
  ...
  void relax() {
    float vx=to1.x-from.x;
    float vy=to1.y-from.y;
    float d=mag(vx, vy);
    if (d>0) {
      float f=(len-d)/(d*5);
      float dx=f*vx;
  ...
  void draw() {
    stroke(edgeColor);
    strokeWeight(0.35);
    line(from.x, from.y, to1.x, to1.y);
  }
}
